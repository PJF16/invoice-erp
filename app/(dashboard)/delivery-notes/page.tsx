  ...
  const [notes, customers, session] = await Promise.all([
    prisma.deliveryNote.findMany({
      ...
      include: {
        ...
        lines: { select: { billingStatus: true, quantity: true, canceledQuantity: true } },
      },
    }),
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {notes.map((note) => {
                ...
                const hasCancellation = note.lines.some((line) => line.canceledQuantity > 0);
                return <tr key={note.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">{canCreateInvoice && <td className="px-3 py-3 text-center">{note.status === "ACTIVE" && pendingCount > 0 && <input type="checkbox" name="lieferscheine" value={note.id} aria-label={`Lieferschein ${note.number} auswählen`} className="h-4 w-4 rounded border-gray-300" />}</td>}<td className="px-4 py-3 font-medium"><Link href={`/delivery-notes/${note.id}`} className="text-blue-700 hover:underline">{note.number}</Link></td><td className="px-4 py-3">{note.customerName}</td><td className="px-4 py-3 text-gray-500">{formatDate(note.issueDate)}</td><td className="px-4 py-3 text-gray-500">{note.deliveryMethod === "STOCK" ? "Eigenes Lager" : `Direkt · ${note.distributor}`}</td><td className="px-4 py-3 text-right tabular-nums">{note._count.lines}</td><td className="px-4 py-3">{note.status === "CANCELED" ? <span className="text-red-700">Storniert</span> : hasCancellation ? <span className="text-amber-700">Teilweise storniert · {pendingCount} offen</span> : pendingCount === 0 ? <span className="text-green-700">Erledigt</span> : pendingCount === note._count.lines ? <span className="text-amber-700">Ausstehend</span> : <span className="text-amber-700">{pendingCount} offen</span>}</td><td className="px-4 py-3 text-gray-500">{note.createdBy.name}</td>{canCreateInvoice && <td className="px-4 py-3 text-right">{note.status === "ACTIVE" && pendingCount > 0 ? <Link href={`/invoices/new?lieferscheine=${note.id}`} className="rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100">In Rechnung übernehmen</Link> : <span className="text-xs text-gray-400">–</span>}</td>}</tr>;
              })}
            </tbody>
          </table>
        </div>
