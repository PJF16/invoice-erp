...
export default async function DeliveryNoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    prisma.deliveryNote.findUnique({
      ...
      include: {
        ...
        cancellations: {
          ...
          include: { canceledBy: { select: { name: true } }, lines: { include: { deliveryNoteLine: { select: { position: true, itemName: true } } } } },
        },
  ...
  const cancelableLines = note.lines
    .filter((line) => !line.invoiceLine && !line.movement?.invoiceLine && line.billingStatus !== "INVOICED" && line.billingStatus !== "CANCELED" && line.canceledQuantity < line.quantity)
    .map((line) => ({ id: line.id, position: line.position, itemName: line.itemName, itemSku: line.itemSku, remainingQuantity: line.quantity - line.canceledQuantity }));
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {note.cancellations.length > 0 && <section className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm"><h2 className="font-semibold text-amber-900">Stornoverlauf</h2><div className="mt-3 space-y-3">{note.cancellations.map((cancellation) => <div key={cancellation.id} className="border-t border-amber-200 pt-3 first:border-0 first:pt-0"><p className="font-medium text-amber-900">{formatDate(cancellation.canceledAt)} · {cancellation.canceledBy.name}</p><p className="text-amber-800">{cancellation.lines.map((line) => `Pos. ${line.deliveryNoteLine.position}: ${line.quantity} × ${line.deliveryNoteLine.itemName}`).join(" · ")}</p><p className="mt-1 whitespace-pre-line text-gray-600">{cancellation.reason}</p></div>)}</div></section>}
      <section className="mt-6 overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="w-full text-sm"><thead><tr className="border-b border-gray-200 text-left text-xs uppercase tracking-wide text-gray-500"><th className="px-4 py-3">Pos</th><th className="px-4 py-3">SKU</th><th className="px-4 py-3">Artikel</th><th className="px-4 py-3">Lager</th><th className="px-4 py-3 text-right">Menge</th><th className="px-4 py-3">Verrechnung</th></tr></thead>
          <tbody>{note.lines.map((line) => { const invoiceLine = line.invoiceLine ?? line.movement?.invoiceLine; return <tr key={line.id} className="border-b border-gray-100 last:border-0"><td className="px-4 py-3 text-gray-500">{line.position}</td><td className="px-4 py-3 font-mono text-xs text-gray-500">{line.itemSku ?? "–"}</td><td className="px-4 py-3 font-medium">{line.itemName}</td><td className="px-4 py-3 text-gray-500">{line.warehouseName ?? "Direktversand"}</td><td className="px-4 py-3 text-right font-semibold tabular-nums">{line.quantity}{line.canceledQuantity > 0 && <span className="block text-xs font-normal text-red-600">{line.canceledQuantity} storniert · {line.quantity - line.canceledQuantity} offen</span>}</td><td className="px-4 py-3">{invoiceLine ? <Link href={`/invoices/${invoiceLine.invoice.id}`} className="text-blue-700 hover:underline">{invoiceLine.invoice.number ?? "Rechnungsentwurf"}</Link> : line.billingStatus === "CANCELED" ? <span className="text-red-700">Storniert</span> : line.billingStatus === "GIFTED" ? <span className="text-green-700">Verschenkt</span> : line.canceledQuantity > 0 ? <span className="text-amber-700">Teilweise storniert</span> : <span className="text-amber-700">Ausstehend</span>}</td></tr>; })}</tbody>
        </table>
      </section>
