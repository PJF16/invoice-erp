...
export default async function CustomerHandoversPage({ searchParams }: { searchParams: Promise<{ status?: string; kunde?: string }> }) {
  ...
  const [movements, customers, counts] = await Promise.all([
    ...
    prisma.customer.findMany({ where: { movements: { some: { billingStatus: { not: null } } } }, orderBy: { name: "asc" }, select: { id: true, name: true } }),
  ...
  return <div className="mx-auto max-w-7xl">
    <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
      ...
      <form method="GET" className="flex flex-wrap gap-2">
        <select name="kunde" defaultValue={kunde ?? ""} className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm"><option value="">Alle Kunden</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select>
