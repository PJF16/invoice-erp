...
export const openApiDocument = {
  ...
  paths: {
    ...
    "/api/offers/{id}": path({
      ...
      patch: operation("Angebote", "Angebot unabhängig vom Status vollständig ändern", { parameters: [id], body: "OfferInput" }),
      delete: operation("Angebote", "Angebotsentwurf löschen", { parameters: [id] }),
    }),
    "/api/offers/{id}/finalize": path({ post: operation("Angebote", "Angebot finalisieren und Nummer vergeben", { parameters: [id], body: "FinalizeOfferInput", bodyRequired: false }) }),
  ...
  components: {
    ...
    schemas: {
      ...
      OfferInput: { type: "object", required: ["customerId", "issueDate", "validUntil", "lines"], properties: { number: { type: ["string", "null"], maxLength: 100, description: "Optionale eigene Angebotsnummer. Bei finalisierten Angeboten erforderlich; wird beim PATCH ohne Feld unverändert beibehalten." }, customerId: { type: "string" }, issueDate: { type: "string", format: "date" }, validUntil: { type: "string", format: "date" }, deliveryDate: { type: ["string", "null"], format: "date" }, servicePeriodStart: { type: ["string", "null"], format: "date" }, servicePeriodEnd: { type: ["string", "null"], format: "date" }, taxTreatment: { $ref: "#/components/schemas/TaxTreatment" }, notes: { type: ["string", "null"] }, lines: { type: "array", minItems: 1, items: { $ref: "#/components/schemas/DocumentLine" } } } },
      FinalizeOfferInput: { type: "object", description: "Optionale eigene Angebotsnummer und optionales Angebotsdatum. Nicht angegebene Werte werden aus dem Entwurf übernommen bzw. automatisch vergeben.", properties: { number: { type: "string", minLength: 1, maxLength: 100 }, issueDate: { type: "string", format: "date" } } },
