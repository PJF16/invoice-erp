...
- Rechnungslogik zentral in `lib/invoices.ts` (`computeTotals`, `finalizeInvoice`, `createStornoInvoice`): Nummernvergabe atomar über `CompanySettings.lastInvoiceSeq`, Hardware-Positionen buchen via `bookMovementTx()` innerhalb derselben Transaktion. Kundendaten werden bei Finalisierung als Snapshot eingefroren.
...
- Storno: nie nur den Status auf CANCELED setzen — immer `createStornoInvoice()` (erzeugt CREDIT_NOTE-Beleg mit negierten Beträgen, bucht Hardware zurück). Umsatzauswertungen zählen nur `type=INVOICE` mit Status OPEN/SENT/PAID (Storno-Paar hebt sich damit auf).
- Mahnwesen: `lib/reminders.ts` (`sendReminderEmail`, `runAutoReminders` im Scheduler); überfällig = `overdueWhere()`. Startseite `/` ist das Dashboard, der Lagerbestand liegt unter `/stock`.
- SMTP-Transport zentral in `lib/mail-transport.ts` (`getMailTransport`, `fillMailTemplate`) — von `mailer.ts`, `reminders.ts` und `export-schedule.ts` genutzt, nicht duplizieren. Intervall-Arithmetik zentral in `lib/dates.ts` (`addInterval`), genutzt von `recurring.ts` und `export-schedule.ts`.
- Export: `lib/export.ts` (`computePeriodRange`, `queryExportInvoices`, `buildDocumentsZip` via `archiver`), `lib/export-schedule.ts` (`runExportSchedule`, `runDueExportSchedules` im Scheduler). Export umfasst nur finalisierte Belege (`number: { not: null }`), nie Entwürfe.
